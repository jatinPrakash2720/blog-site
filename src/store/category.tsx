...
const CategoryContext = createContext<
  contextInterfaces.ICategoryContext | undefined
>(undefined);
...
export const CategoryProvider: React.FC<
  contextInterfaces.CategoryProviderProps
> = ({ children }) => {
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);

  const [topLevelCategories, setTopLevelCategories] = useState<
    apiInterfaces.Category[]
  >([]);
  ...
  const createSubCategory = useCallback(
    async (parentId: string, data: apiInterfaces.CreateSubCategoryPayload) => {
      let success = false;
      await requestHandler(
